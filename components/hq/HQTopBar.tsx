  ...
  return (
    <header className="hidden lg:flex fixed top-0 left-64 right-0 z-20 h-12 bg-[#0b0c0d]/90 backdrop-blur-sm border-b border-white/[0.05] items-center justify-end px-6 gap-1">
      <div
        ...
        className="flex h-8 cursor-not-allowed items-center gap-2 border border-dashed border-white/[0.05] px-3 text-[10px] uppercase tracking-[0.2em] text-white/15"
      >
        <Search size={11} />
        <span>Search</span>
      </div>
      ...
      {disabledControls.map(({ Icon, label }) => (
        <div
          ...
          className="flex h-8 w-8 cursor-not-allowed items-center justify-center text-white/15"
        >
          <Icon size={14} />
        </div>
      ))}
      ...
      <div
        ...
        className="flex h-8 cursor-not-allowed items-center gap-2 px-2.5 text-white/15"
      >
