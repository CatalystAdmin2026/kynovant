...
//   STRIPE_MONTHLY_PRICE_ID — required for coach checkout to work at
//   all. Create the Product + a recurring monthly Price in the Stripe
//   Dashboard, then set this to that Price's id (price_...).
//
...
// (used by the Stripe webhook to distinguish a Kynovant coach-platform
// event from a Catalyst Elite end-client coaching-package event) is
// derived entirely from this registry, so a newly configured plan is
// automatically recognized by both.
//
 ...
 * plan above — never a Catalyst Elite end-client coaching-package
 * price, and never true for an unconfigured or unrecognized price id.
 * Used by the Stripe webhook (app/api/stripe/webhook/route.ts) to route
 * customer.subscription.* / invoice.* events to the coach-subscription
 * sync path (lib/billing/sync.ts) instead of the existing client-
 * payment path. Replaces the former STRIPE_COACH_PLAN_PRICE_IDS
 * comma-separated env var — that was a placeholder for exactly this
 * registry, never set in any real environment, and a free-form list
 * would let the app "know" a price ID without a corresponding named
 * plan, which is what this registry exists to prevent.
 */
